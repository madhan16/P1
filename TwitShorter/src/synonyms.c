...
void find_synonyms_to_tweet(twitter_words_tbl *wl, int words_in_tweet) {
    ...
    for (i = 0; i < words_in_tweet; i++) {
        start_byte = find_word_in_file(ifp, wl[i].word, &longest_line);
        //printf("Word: %s Start_byte: %d\n", wl[i].word, start_byte);
        if (start_byte > -1) {
            amt_shorter_synonyms = get_synonyms_from_fileline(ifp, start_byte, longest_line, shorter_synonyms);
...
long find_word_in_file(FILE *ifp, char *search_for, unsigned long *out_longest_line) {
	...
	unsigned long longest_line;
	unsigned long *line_positions = index_lines_in_file(ifp, &number_of_lines, &longest_line);
    
	long line_number = binary_search(ifp, line_positions, longest_line, 0, number_of_lines - 1, search_for);

	start_byte = -1;

	if (line_number > -1) {
		start_byte = line_positions[line_number];
	}
...
unsigned long *index_lines_in_file(FILE *ifp, unsigned long *out_number_of_lines, 
                                   unsigned long *out_longest_line) {
...
void read_word_in_line(FILE *ifp, unsigned long line_start_byte, unsigned long longest_line, 
                       char *out_word) {
...
void print_shorter_synonyms(synonym_tbl *shorter_synonyms, int amt_shorter_synonyms) {
    int i;
    printf("Sugested synonyms for: %s\n", shorter_synonyms[0].word);
